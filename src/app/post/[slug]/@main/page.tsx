...
const MainPost = ({params}) => {
    const { 
      ...
      const { data : similarPostsData } = useFetchSimilarPosts({
        postId: params.slug
      });
      ...
      if (postError || (postData && reactionsError)) {
        const errorMessage = postError?.description || 
      ...
      // Loading state - show skeleton while either query is loading
