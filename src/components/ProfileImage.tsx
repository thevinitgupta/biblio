...
const ProfileImage = ({height=200, width=200, hClass = 'h-12', wClass='w-12', borderClass, children}: {
  ...
  if(!isLoading && (data?.type !== ResponseType.success || isError) ){
    return (
      <div className={`rounded-full flex items-center justify-center text-red-400 ${hClass} ${wClass}`}>
    ...
    return (
      <div className={`rounded-full flex items-center justify-center ${hClass} ${wClass}`}>
  ...
  return (
    <div className={`relative rounded-full overflow-hidden border-primary flex items-center ${hClass} ${wClass} ${borderClass || ""}`}
