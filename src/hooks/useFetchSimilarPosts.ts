import { SimilarPostResponseDataI, SimilarPostsResponseI } from "@/types/common";
...
type UseFetchSimilarPosts = 
  { data: SimilarPostsResponseI | null; error: ErrorResponse 
...
const useFetchSimilarPosts  = ({
    ...
    const { data, error, isLoading } = useQuery<SimilarPostsResponseI>({
        ...
        queryFn: async (): Promise<SimilarPostsResponseI> => {
            ...
            try {
                ...
                const fetchSimilarPosts = await publicClient.get("/posts/similar/" + postId);
